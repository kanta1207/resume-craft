...
import { LayoutGrid, List, PenTool, Search } from "lucide-react";
...
export const Route = createFileRoute("/")({
	component: JdListScreen,
});

function JdListScreen() {
	const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
	const [searchQuery, setSearchQuery] = useState("");

	const filteredJobs = mockJobDescriptions.filter(
		(job) =>
					...
					<div className="flex items-center justify-between gap-8">
						...
						<div className="flex-1 max-w-md relative group">
							...
							<div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
								<div className="text-[10px] font-medium text-muted-foreground/50 border border-border/50 rounded px-1.5 py-0.5 bg-background/50">
									⌘K
								</div>
							</div>
						</div>
				...
				<div className="max-w-7xl mx-auto px-6 py-8">
					...
					<div className="flex items-baseline justify-between mb-6">
						...
						<span className="text-xs font-medium text-muted-foreground bg-muted/50 px-2.5 py-1 rounded-full border border-border/50">
							{filteredJobs.length}{" "}
							{filteredJobs.length === 1 ? "File" : "Files"}
						</span>
					</div>
					...
					>
						{filteredJobs.map((job) => (
							<JdCard key={job.id} job={job} viewMode={viewMode} />
						))}
					</motion.div>
