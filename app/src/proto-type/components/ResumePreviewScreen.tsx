import { ArrowLeft, Download, ChevronDown, Maximize2, Share2 } from 'lucide-react';
import { useState } from 'react';
import { motion } from 'motion/react';
import { Button } from './ui/button';
import { PdfViewer } from './PdfViewer';
import { AtsPanel } from './AtsPanel';
import { mockJobDescriptions, mockAtsAnalysis } from '../data/mockData';
...
export function ResumePreviewScreen({ jdId, onBack, onOpenJdDetail }: ResumePreviewScreenProps) {
  const job = mockJobDescriptions.find(j => j.id === jdId);
  const [selectedVersion, setSelectedVersion] = useState(job?.resumeVersions || 1);
    ...
    >
      ...
      <header className="h-14 border-b border-border/50 bg-background/80 backdrop-blur-xl flex items-center justify-between px-4 z-10">
        <div className="flex items-center gap-4">
          ...
          <button 
            onClick={onOpenJdDetail}
            className="flex items-center gap-2 text-sm group/title hover:bg-muted/50 px-2 py-1 rounded transition-colors text-left"
          >
            <span className="font-semibold text-foreground group-hover/title:text-primary transition-colors">{job.title}</span>
            <span className="text-muted-foreground opacity-50">•</span>
            <span className="text-muted-foreground group-hover/title:text-foreground/80 transition-colors">{job.company}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          {job.resumeVersions && job.resumeVersions > 1 && (
            <div className="relative mr-2">
              ...
              >
                {Array.from({ length: job.resumeVersions }, (_, i) => i + 1).map((v) => (
