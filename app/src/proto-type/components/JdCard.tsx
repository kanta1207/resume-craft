import { Calendar, MapPin, ArrowRight, FileCheck } from 'lucide-react';
...
export function JdCard({ job, viewMode, onOpenPreview, onOpenDetail }: JdCardProps) {
  ...
  return (
    <div className="group relative flex flex-col bg-card hover:bg-muted/30 border border-border/40 rounded-2xl overflow-hidden transition-all duration-300 hover:border-primary/20 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
      ...
      <div className="p-5 pb-0 flex items-start justify-between gap-4">
        ...
        {hasResume && (
          <Badge variant="outline" className="bg-emerald-500/5 text-emerald-600 border-emerald-500/20 px-2 py-0.5 h-6 font-medium text-[10px] uppercase tracking-wider">
            Ready
          </Badge>
        )}
      </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto pt-4 border-t border-border/40 flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={onOpenDetail}
            className="flex-1 bg-transparent hover:bg-muted/50 border-border/50 hover:border-primary/30 text-xs font-medium h-8"
          >
            Details
          </Button>
          <Button
            ...
            disabled={!hasResume}
            className={`flex-1 text-xs font-medium h-8 shadow-none transition-all ${
              hasResume 
