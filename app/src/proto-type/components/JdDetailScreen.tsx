import { ArrowLeft, Eye, Calendar, MapPin, Briefcase, Building2, DollarSign, Clock, FileCheck, ChevronRight } from 'lucide-react';
import { motion } from 'motion/react';
import { Button } from './ui/button';
import { Badge } from './ui/badge';
import { mockJobDescriptions } from '../data/mockData';

interface JdDetailScreenProps {
  jdId: string;
  onBack: () => void;
  onGenerateResume: () => void;
}

export function JdDetailScreen({ jdId, onBack, onGenerateResume }: JdDetailScreenProps) {
        ...
        <div className="max-w-5xl mx-auto p-6 md:p-10 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-start gap-8 mb-12 pb-8 border-b border-border/40">
            ...
            <div className="flex-1">
              <h1 className="text-3xl font-bold tracking-tight mb-4 text-foreground">{job.title}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-10">
              <section>
                ...
                <div className="prose prose-zinc max-w-none text-muted-foreground leading-relaxed text-sm">
                  <p>{job.description}</p>
                  <p className="mt-4">
                    We're looking for an exceptional engineer to join our growing team and help shape the future of 
                    our product. You'll work on challenging problems at scale, collaborate with talented engineers, 
                    and have the opportunity to make a significant impact on our platform.
                  </p>
                </div>
              </section>
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-border/50 bg-card p-6 space-y-6">
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-4">Resume Status</h4>
                  {job.hasResume ? (
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                      ...
                      <div>
                        <div className="text-sm font-medium text-emerald-700">Ready to view</div>
                        <div className="text-xs text-emerald-600/80">{job.resumeVersions} versions available</div>
                      </div>
                    </div>
                  ) : (
                     <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border/50">
                      ...
                      <div>
                        <div className="text-sm font-medium text-muted-foreground">No resume yet</div>
                        <div className="text-xs text-muted-foreground/70">Generate via CLI to analyze</div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {!job.hasResume && (
                    <p className="text-xs text-center text-muted-foreground">
                      Switch to your code editor or CLI to generate this resume with AI.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-border/50 bg-card p-6">
                <h4 className="text-sm font-medium text-foreground mb-4">Nice to Have</h4>
                <ul className="space-y-3">
                  {[
